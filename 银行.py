...
def adduser():
    ...
    door = input('\t\t请输入你的门牌号：')
    add = bank_add(username, account, password, country, province, street, door)
    if add == 1:
        ...
        info = '''
                        ...
                        '''
        # 每个元素都可传入%
        print(info % (username, account, country, province, street, door, bank[username]['存款余额'], bank_name))
    elif add == 2:
        print('用户已存在')
    elif add == 3:
...
def bank_add(username, account, password, country, province, street, door):
    if username in bank:
        return 2
    elif len(bank) >= 100:
        return 3
